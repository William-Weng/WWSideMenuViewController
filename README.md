...
```bash
dependencies: [
    .package(url: "https://github.com/William-Weng/WWSideMenuViewController.git", .upToNextMajor(from: "1.4.3"))
]
```
...
|函式|說明|
|-|-|
|initSettingWithSegue(_:displayPosition:visualEffectStyle:cornerRadius:delegate:)|初始化設定 (使用Segue)|
|initSettingWithViewController(_:displayPosition:visualEffectStyle:cornerRadius:delegate:)|初始化設定 (使用UIViewController)|
